    ...
    public void createTaskSaveButtons(Composite parent, boolean horizontal, int hSpan) {
        if (getContainer().isSelectorMode()) {
            ...
            if (supportsVariables) {
                UIUtils.createDialogButton(panel, TaskUIMessages.task_config_wizard_group_task_variables, new SelectionAdapter() {
                    @Override
            ...
            saveAsTaskButton = UIUtils.createDialogButton(panel, TaskUIMessages.task_config_wizard_group_save_task, new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    saveTask();
                }
            });
            Link tasksLink = UIUtils.createLink(panel, "<a>" + TaskUIMessages.task_config_wizard_group_open_tasks_view + "</a>", new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    try {
                        UIUtils.getActiveWorkbenchWindow().getActivePage().showView(DatabaseTasksView.VIEW_ID);
                    } catch (PartInitException e1) {
                        DBWorkbench.getPlatformUI().showError(TaskUIMessages.task_config_wizard_group_open_tasks_view_error_title, TaskUIMessages.task_config_wizard_group_open_tasks_view_error_description, e1);
                    }
                }
            });
